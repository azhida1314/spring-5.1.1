...
public class ConfigurationClassPostProcessor implements BeanDefinitionRegistryPostProcessor,
	...
	/**
	 * 解析配置类  获取需要被扫描的bean的定义信息 放大 bdMap中
	...
	public void processConfigBeanDefinitions(BeanDefinitionRegistry registry) {
		//存储需要处理的配置的
		//BeanDefinitionHolder 就是 BeanDefinition 和  beanName
		List<BeanDefinitionHolder> configCandidates = new ArrayList<>();
		//所有需要通过工厂实例化的bean的名称
		//这时 candidateNames 应该没有几个  其中有用的就是我门通过 context.registroy 注册进去的配置类
		String[] candidateNames = registry.getBeanDefinitionNames();

		for (String beanName : candidateNames) {
			//获取bean的定义信息
			BeanDefinition beanDef = registry.getBeanDefinition(beanName);
			//判断是否是全注解类 或者 iteConfigurationClass
			//如果是的话证明这个配置类被解析过  因为解析时会修改bean的定义信息设置这个字端
			if (ConfigurationClassUtils.isFullConfigurationClass(beanDef) ||
				...
				//第一次 走这个判断逻辑
				//检查给定的bean定义是否满足配置类的要求   全注解的类 @Configuration 和非全注解的类 Component  ComponentScan  Import  ImportResource 类中有@Bean的注解
				//如果是标注了@component 直接会放入到bdm里面
		...
		do {
			...
			//  在1出解析后会有一些放在集合中  还没有处理   在这一步  把那些需要处理的bd放入到bdMap中
