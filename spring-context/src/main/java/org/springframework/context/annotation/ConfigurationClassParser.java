...
class ConfigurationClassParser {
	...
	public void parse(Set<BeanDefinitionHolder> configCandidates) {
		...
		//如果有延迟处理的额  ImportSelector 在此处处理
		//没用过
	 ...
	 * 解析配置类
	 *
	 * @param configClass 属性的封装 便于传参
	 * @throws IOException
	 */
	protected void processConfigurationClass(ConfigurationClass configClass) throws IOException {
		//是否应该跳过  条件的使用 如果被跳过  则不会被放入到 bdMap中 最后也不会实例化
		//条件的使用   springBoot 中大量的注解自动配置 在这做的校验
	...
	protected final SourceClass doProcessConfigurationClass(ConfigurationClass configClass, SourceClass sourceClass)
		...
		for (MethodMetadata methodMetadata : beanMethods) {
			//给配置类添加 BeanMethod
			//后面会根据  loadBeanDefinitionsForBeanMethod 这个产生对象
				...
				for (SourceClass candidate : importCandidates) {
					...
					if (candidate.isAssignable(ImportSelector.class)) {
						...
						if (selector instanceof DeferredImportSelector) {
							...
							//递归 检查有没有被import 没有的话 会当作import的普通类做处理
						...
						//添加到集合中  在外部再加入到 bdmap中
						configClass.addImportBeanDefinitionRegistrar(registrar, currentSourceClass.getMetadata());
					} else {
						//import普通类的解析 作为一个配置类  重新走一遍解析流程
